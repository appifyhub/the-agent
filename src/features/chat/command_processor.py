...
class CommandProcessor:
    ...
    @dataclass
    class Result:
        status: Literal["failed", "ignored", "success"]
        ...
        try:
            ...
            if command_parts and command_parts[0].startswith("/"):
                ...
                if command_name in SUPPORTED_COMMANDS:
                    status = self.__handle_config_commands(command_name, command_args)
    ...
    def __handle_config_commands(self, command_name: str, command_args: list[str]) -> Literal["failed", "ignored", "success"]:

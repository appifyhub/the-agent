...
class ToolResolutionError(Exception):
    def __init__(self, tool_type: ToolType, user_id: str):
        message = f"Unable to resolve a tool for '{tool_type.value}' for user '{user_id}'. "
...
class ToolChoiceResolver(SafePrinterMixin):
    ...
    def __get_user_tool_choice(self, tool_type: ToolType) -> str | None:
        match tool_type:
            case ToolType.chat:
    ...
    def get_prioritized_tools(
        ...
        if resolved_user_choice_tool and tool_type in resolved_user_choice_tool.types:
        ...
        if (
            ...
            and tool_type in resolved_default_tool.types

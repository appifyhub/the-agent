...
class PriceAlertManager(SafePrinterMixin):
    class TriggeredAlert(BaseModel):
        ...
        old_rate: float
        old_rate_time: datetime
        new_rate: float
        new_rate_time: datetime
    ...
    def create_alert(self, base_currency: str, desired_currency: str, threshold_percent: int) -> PriceAlert:
        ...
        return PriceAlert.model_validate(price_alert_db)
    ...
    def get_all_alerts(self) -> list[PriceAlert]:
        ...
        return [PriceAlert.model_validate(alert) for alert in price_alerts_db]

    def delete_alert(self, base_currency: str, desired_currency: str) -> PriceAlert | None:
        self.sprint(f"Deleting price alert for {base_currency}/{desired_currency}")
        deleted_alert = self.__price_alert_dao.delete(self.__chat_config.chat_id, base_currency, desired_currency)
        return PriceAlert.model_validate(deleted_alert) if deleted_alert else None
